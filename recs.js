...
//   const topicString = Array.isArray(post.topic) ? post.topic.join(' ') : post.topic;
...
function createArticleCard(post, index) {
    ...
    const topicHTML = Array.isArray(post.topic)
      ...
      const genreHTML = Array.isArray(post.genre)
      ? post.genre.map(t => `<span class="genre-badge">${t}</span>`).join(' ')
      : `<span class="genre-badge">${post.genre}</span>`;
  

    item.innerHTML = `
      <a href="${post.link}" class="card-bp" target="_blank">
        <div class="thumb" style="background-image: url('./images/recs/${post.image}');"></div>

        <article>
          <h3 style="margin: -10px 0px 10px 0px; text-align: center">< ${post.rating} ></h3>
          <h1>${post.title}</h1>
          ${post.description ? `<p>${post.description}</p>` : ""}
          <div class="topic-container">${topicHTML} ${genreHTML}</div>
        </article>
      </a>
    `;
    return item;
...
// Load JSON data
fetch('recs.json') // Replace file name as needed
  .then(res => res.json())
  .then(data => {
